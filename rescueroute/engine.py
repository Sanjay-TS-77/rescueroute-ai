...
from .store import (
    RECIPIENTS,
    VOLUNTEERS,
    list_events as store_list_events,
    load_event,
    next_event_id,
    save_event,
)
...
def _save(event: dict[str, Any]) -> dict[str, Any]:
    save_event(event)
    return event
...
def assign_volunteer(event_id: str) -> dict[str, Any]:
    ...
    unavailable = set(event.get("unavailable_volunteers", []))
    candidates = [
        volunteer for volunteer in VOLUNTEERS
        if volunteer["available"]
        and volunteer["id"] not in unavailable
        and volunteer["max_km"] >= route_km
    ]
    if not candidates:
...
def cancel_assigned_volunteer(event_id: str) -> dict[str, Any]:
    ...
    unavailable = set(event.get("unavailable_volunteers", []))
    unavailable.add(volunteer_id)
    event["unavailable_volunteers"] = sorted(unavailable)

    # Preserve the local-demo behavior while event-level unavailability makes
    # cancellation durable across separate AgentCore instances.
...
def recover_logistics(event_id: str) -> dict[str, Any]:
    ...
    candidates = [
        ...
        if v["available"] and v["id"] not in unavailable and v["max_km"] >= route_km
    ]
